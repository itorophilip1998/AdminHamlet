  ...
  export default {
    ...
    link(email)
    {
       ...
       return  moment(time).format("YYYY-MM-DD") 
        ...
        getuser()
        {
         this.$http.get(`${this.$baseApi}/ban/users`,{headers:{'Authorization':`Bearer ${localStorage.getItem(this.$token)}`}}).then((response)=> {
               this.users=response.data.banned_users.data
               console.log(this.users) 
               this.length=response.data.banned_users.total
         }) 
        }
    },
  }
